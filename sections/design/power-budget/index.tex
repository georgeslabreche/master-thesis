...
\input{sections/design/power-budget/tables/flat-terrain-global-minimum-maximum-and-medium-power-draws.tex}
...
\input{sections/design/power-budget/tables/upslope-terrain-global-minimum-maximum-and-medium-power-draws.tex}
...
\input{sections/design/power-budget/tables/upslope-terrain-local-media-measurement-summary.tex}

\subsection{Traverse Power Budget}
\label{sec:PowerBudget:PowerBudget:TraversePowerBudget}
Worst-case flat traverse times were set to \SI{15}{\minute} at Iani Chaos and \SI{5}{\minute} at Ismenius Cavus. These durations were determined during the \ac{SA} sizing process covered in Section \ref{sec:Design:Simulation}. Differences in traverse time were a result of available sunlight at both sites on their respective worst days with respect to available daily insolation. Ismenius Cavus is located at a higher latitude, in the northern hemisphere, and thus receives less winter season daylight than Iani Chaos. Worst-case daily insolations for an optical depth of $\tau = 1$ were used to determine the energy requirements of each reference Sol and were taken from Tables \ref{tab:insolation-iani-chaos-clear-and-dusty-days} for Iani Chaos and \ref{tab:insolation-ismenius-cavus-clear-and-dusty-days} for Ismenius Cavus.

\subsubsection{Flat Traverse}
\label{sec:Design:PowerBudget:TraversePowerBudget:FlatTraverse}

Power and operation al duration of \textit{DTE Communication}, \textit{Science Stop - Short}, and \textit{Hibernation} modes were taken from \citeother{CDF2014} and assigned to the reference Sols presented in Section \ref{sec:ReferenceSols:ReferenceSols}. Power draw estimates made in Sections \ref{sec:PowerBudget:PropulsionPowerBudget:FlatTerrainTraverse} and \ref{sec:PowerBudget:PropulsionPowerBudget:UpslopeTerrainTraverse} were used for \textit{Traverse} modes power. The \textit{Optimal Pose} mode power draw was equated to that of the rover's flat terrain propulsion power and etimated to take up to \SI{10}{\minute} to execute. The power budget for the reference \textit{Flat Traverse Sol} is shown in Table XX.

\input{sections/design/power-budget/tables/power-budget-tau-1-traverse-sol-flat.tex}



\subsubsection{Upslope Traverse}
\label{sec:Design:PowerBudget:TraversePowerBudget:UpslopeTraverse}
The reference Sol for an upslope traverse only differs from that of the flat traverse by the \textit{Traverse} mode power draw. It is increased from \SI{75}{\watt} to \SI{150}{\watt}. The power budget for the reference \textit{Upslope Traverse Sol} is shown in Table XX.

\input{sections/design/power-budget/tables/power-budget-tau-1-traverse-sol-upslope.tex}

\subsubsection{Hibernation}
\label{sec:Design:PowerBudget:TraversePowerBudget:Hibernation}
The power budget for the reference \textit{Hibernation Sol} is shown in Table XX.
