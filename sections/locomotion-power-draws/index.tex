This chapter details the power budget for the reference Sols used in mission planning. Power budgets for the rover modes comprising the reference Sols were taken from \ac{ESA}'s \ac{CDF} Study Report on MarsFAST. Propulsion power budgets for the rover's Traverse mode were determined from data collected during the SherpaTT field trials in Utah.
...
\clearpage
\input{sections/locomotion-power-draws/tables/flat-terrain-global-minimum-maximum-and-medium-power-draws.tex}
...
\clearpage
\input{sections/locomotion-power-draws/tables/upslope-terrain-global-minimum-maximum-and-medium-power-draws.tex}
...
\clearpage
\input{sections/locomotion-power-draws/tables/upslope-terrain-local-media-measurement-summary.tex}

Based on data from the SherpaTT field campaign and the initial assumptions made in Section \ref{sec:PropulsionPowerConstraints:Introduction}, the following assumption were made with respect to propulsion power draws:

\begin{itemize}
    \item \SI{75}{\watt} for flat terrain traverses.
    \item \SI{150}{\watt} for upslope terrain traverses up to \SI{30}{\degree} inclination.
\end{itemize}

\section{Rover Mode Power Budget}
\label{sec:PowerBudget:RoverModePowerBudget}
