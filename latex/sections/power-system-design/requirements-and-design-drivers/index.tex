...
\begin{enumerate}[leftmargin=1.31cm, label=\zeroLeadCounter{A}]
    ...
    \item The rover's nominal speed is at least \SI{2}{cm.s^{-1}}.\footnote{Upslope runs of 0.1 \si{m.s^{-1}} and 0.04 \si{m.s^{-1}} were successful with SherpaTT. In comparison, \ac{MER}'s top speed on hard ground was 0.05 \si{m.s^{-1}} with an average speed of 0.01 \si{m.s^{-1}} \citeother{Cordes2018}.}
    \item There is an average \SI{15}{\percent} slippage when traversing a flat terrains for any given distance.
    \item There is an average \SI{20}{\percent} slippage when traversing an upslope terrain for any given distance and slope angle.\footnote{SherpaTT steep slope experiments presented in \citeother{Cordes2018} show a maximum slippage of just above \SI{20}{\percent} for one of the test runs.}
\end{enumerate}
